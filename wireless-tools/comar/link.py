...
class Wireless:
    ...
    def setEncryption(self, mode=None, parameters=None):
        ...
        if mode == "wep":
            ...
            pass
        elif mode == "wpa-enterprise":
            if not supplicant:
                fail(_(MSG_NO_SUPPLICANT))
            if not wpa_supplicant.startWpaService():
                fail(_(MSG_NO_SUPPLICANT_SERVICE))
            try:
                ret = wpa_supplicant.setWpaEnterpriseAuthentication(self.ifc.name, self.ssid, parameters)
            except:
                return
            if not ret:
                fail(_(MSG_WPA_FAILED))
...
def authMethods():
    return [
        ...
        ("wpa-enterprise", _(AUTH_WPA_ENTERPRISE)),
    ]
...
def authParameters(mode):
    if mode in ("wep", "wepascii", "wpa-psk"):
        ...
        return [
            ("key_mgmt", "key_mgmt", "text"),
            ("pairwise", "pairwise", "text"),
            ("group", "group", "text"),
            ("eap", "eap", "text"),
            ("phase2", "phase2", "text"),
            ("anonymous_identity", "anonymous_identity", "text"),
